...
export function encodeControlCommand(command) {
  try {
    ...
    let commandType;
    switch (command.type) {
      case 'mousemove': commandType = COMMAND_TYPE.MOUSE_MOVE; break;
      case 'mousedrag': commandType = COMMAND_TYPE.MOUSE_DRAG; break;
      case 'mousedown': commandType = COMMAND_TYPE.MOUSE_DOWN; break;
      case 'mouseup': commandType = COMMAND_TYPE.MOUSE_UP; break;
      case 'mouseclick': commandType = COMMAND_TYPE.MOUSE_CLICK; break;
      case 'doubleclick': commandType = COMMAND_TYPE.DOUBLE_CLICK; break;
      case 'contextmenu': commandType = COMMAND_TYPE.CONTEXT_MENU; break;
      case 'longpress': commandType = COMMAND_TYPE.LONG_PRESS; break;
      case 'scroll': commandType = COMMAND_TYPE.SCROLL; break;
      case 'keydown': commandType = COMMAND_TYPE.KEY_DOWN; break;
      case 'keyup': commandType = COMMAND_TYPE.KEY_UP; break;
      case 'keypress': commandType = COMMAND_TYPE.KEY_PRESS; break;
      case 'keytype': commandType = COMMAND_TYPE.KEY_TYPE; break;
      case 'shortcut': commandType = COMMAND_TYPE.SHORTCUT; break;
      case 'functionkey': commandType = COMMAND_TYPE.FUNCTION_KEY; break;
      default:
    ...
    if (command.ctrlKey || command.altKey || command.shiftKey || command.metaKey) {
    ...
    if (flags & FLAGS.HAS_BUTTON) {
      ...
      switch (command.button) {
        case 'left': buttonCode = BUTTON_TYPE.LEFT; break;
        case 'middle': buttonCode = BUTTON_TYPE.MIDDLE; break;
        case 'right': buttonCode = BUTTON_TYPE.RIGHT; break;
      }
    ...
    console.log(`[控制协议] 编码完成，大小: ${buffer.length}字节 (原JSON约${JSON.stringify(command).length}字节)`);
    return buffer;
...
export function decodeControlCommand(buffer) {
  try {
    ...
    let type;
    switch (commandType) {
      case COMMAND_TYPE.MOUSE_MOVE: type = 'mousemove'; break;
      case COMMAND_TYPE.MOUSE_DRAG: type = 'mousedrag'; break;
      case COMMAND_TYPE.MOUSE_DOWN: type = 'mousedown'; break;
      case COMMAND_TYPE.MOUSE_UP: type = 'mouseup'; break;
      case COMMAND_TYPE.MOUSE_CLICK: type = 'mouseclick'; break;
      case COMMAND_TYPE.DOUBLE_CLICK: type = 'doubleclick'; break;
      case COMMAND_TYPE.CONTEXT_MENU: type = 'contextmenu'; break;
      case COMMAND_TYPE.LONG_PRESS: type = 'longpress'; break;
      case COMMAND_TYPE.SCROLL: type = 'scroll'; break;
      case COMMAND_TYPE.KEY_DOWN: type = 'keydown'; break;
      case COMMAND_TYPE.KEY_UP: type = 'keyup'; break;
      case COMMAND_TYPE.KEY_PRESS: type = 'keypress'; break;
      case COMMAND_TYPE.KEY_TYPE: type = 'keytype'; break;
      case COMMAND_TYPE.SHORTCUT: type = 'shortcut'; break;
      case COMMAND_TYPE.FUNCTION_KEY: type = 'functionkey'; break;
      default:
    ...
    if (flags & FLAGS.HAS_BUTTON) {
      ...
      offset += 1;
      switch (buttonCode) {
        case BUTTON_TYPE.LEFT: command.button = 'left'; break;
        case BUTTON_TYPE.MIDDLE: command.button = 'middle'; break;
        case BUTTON_TYPE.RIGHT: command.button = 'right'; break;
      }
    }
