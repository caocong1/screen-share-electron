...
function addMouseMoveToQueue(data) {
  ...
  if (mouseMoveBuffer.queue.length > mouseMoveBuffer.maxBatchSize) {
    mouseMoveBuffer.queue = mouseMoveBuffer.queue.slice(-mouseMoveBuffer.maxBatchSize);
  }
    ...
    switch (data.type) {
      ...
      case 'keydown':
        if (data.key) {
          ...
          const keyMap = {
            ...
            'Escape': 'escape', 'Space': 'space', 'CapsLock': 'capslock',
      ...
      case 'shortcut':
        if (data.key) {
          ...
          const keyMap = {
            'c': 'c', 'v': 'v', 'x': 'x', 'z': 'z', 'y': 'y', 'a': 'a', 's': 's',
            'tab': 'tab', 'esc': 'escape', 'l': 'l', 'd': 'd', 'r': 'r', 'space': 'space'
          };
