...
class ScreenShareApp {
  ...
  handleGlobalKeyDown(e) {
    ...
    console.log('[全局键盘按下]', {
      ...
      p2pConnected: p2p.isConnected,
      dataChannelReady:
        p2p.dataChannel && p2p.dataChannel.readyState === 'open',
    });
    ...
    console.log('[全局键盘] 准备发送keydown命令:', command);

    try {
      p2p.sendControlCommand(command);
      console.log('[全局键盘] keydown命令发送成功');
    } catch (error) {
      console.error('[全局键盘] keydown命令发送失败:', error);
    }
  }
  ...
  handleGlobalKeyUp(e) {
    ...
    console.log('[全局键盘] 准备发送keyup命令:', command);

    try {
      p2p.sendControlCommand(command);
      console.log('[全局键盘] keyup命令发送成功');
    } catch (error) {
      console.error('[全局键盘] keyup命令发送失败:', error);
    }
  }
  ...
  bindCanvasMouseEvents() {
    ...
    this.canvasMouseHandlers.click = (event) => {
      ...
      console.log(
        '[Canvas鼠标] 点击事件已通过 mousedown + mouseup 处理，无需额外发送',
      );
    };
  ...
  bindPointerLockEvents() {
    ...
    this.pointerLockClickHandler = (event) => {
      ...
      // 点击事件的处理已经通过 mousedown 和 mouseup 完成
      console.log(
        '[指针锁定] 点击事件已通过 mousedown + mouseup 处理，无需额外发送',
      );
    };
  ...
  sendMouseCommand(type, coords, extra = {}) {
    ...
    const command = {
      ...
      isDragging:
        extra.isDragging !== undefined ? extra.isDragging : this.isDragging, // 优先使用 extra 中的值
    };
